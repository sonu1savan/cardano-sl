...
verifyToil bvd blacklist curEpoch verifyAllIsKnown =
    mapM (verifyAndApplyTx bvd blacklist curEpoch verifyAllIsKnown . withTxId)
...
processTx bvd blacklist curEpoch tx@(id, aux) = do
    whenM (lift $ hasTx id) $ throwError ToilKnown
    whenM ((>= memPoolLimitTx) <$> lift memPoolSize) $
        throwError (ToilOverwhelmed memPoolLimitTx)
    undo <- mapExceptT utxoMToLocalToilM $ verifyAndApplyTx bvd blacklist curEpoch True tx
...
normalizeToil ::
    ...
    -> [(TxId, TxAux)]
    -> LocalToilM ()
normalizeToil bvd blacklist curEpoch txs = mapM_ normalize ordered
  where
    ...
    normalize ::
        ...
        => (TxId, TxAux)
        -> LocalToilM ()
    normalize = void . runExceptT . processTx bvd blacklist curEpoch
...
verifyAndApplyTx adoptedBVD blacklist curEpoch verifyVersions tx@(_, txAux) = do
    whenLeft (checkTxAux txAux) (throwError . ToilInconsistentTxAux)
    vtur@VerifyTxUtxoRes {..} <- Utxo.verifyTxUtxo ctx blacklist txAux
...
verifyGState bvd@BlockVersionData {..} curEpoch txAux vtur = do
    ...
    unless (isRedeemTxUndo $ vturUndo vtur) $ whenJust txFeeMB $ \txFee ->
        verifyTxFeePolicy txFee bvdTxFeePolicy txSize
    when (txSize > limit) $
        throwError $ ToilTooLargeTx txSize limit
  where
    -- This is confusing. Why? Do transactions that come from redeem addresses
    -- not have to pay fees?
    -- Why List.all instead of List.any?
    isRedeemTxUndo :: TxUndo -> Bool
    isRedeemTxUndo resolvedOuts =
        all isRedeemAddress
