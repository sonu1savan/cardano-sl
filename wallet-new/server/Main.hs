...
import           Pos.Launcher (NodeParams (..), NodeResources (..), bpLoggingParams,
                               ...
                               withConfigurations)
import           Pos.Launcher.Configuration (BlacklistPath (..), ConfigurationOptions, HasConfigurations)
...
startEdgeNode wso =
  ...
  where
    ...
    blPath :: Maybe BlacklistPath
    blPath = BlacklistPath <$> CLI.cnaBlacklistPath (wsoNodeArgs wso)
